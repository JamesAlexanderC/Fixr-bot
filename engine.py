...
async def login_get_ticket(
    ...
    try:
        ...
        logger.error("Error getting in on %s account: %s", email, str(e), exc_info=True)
        raise e
...
async def queueHandler():
    ...
    while not stopEvent.is_set():
        ...
        if msg == "start-scan-loop":
            logger.debug("Message recongised as 'start-scan-loop'")
            if scan_session != {}:
                logger.warning("Scan is already active, IGNORING")
            try:
                # currently assuming the accounts file exists with at least one entry - I will change this
                ...
                continue
            try:
                # same as above with assuming file exists
            ...
            for account in accounts.keys():
                try:
                    ...
                    logger.error("Error creating ticket task for %s: %s", account, str(e), exc_info=True)
        elif msg.split("|")[0] == "buy-ticket":
            logger.info("Message is recognised as 'buy-tickets'")
            # I rlly need to sort this out
            try:
            ...
            # NEED TO EDIT AND TEST NEW VERSION OF BUY-TICKETS THEN INSERT HERE, THEN CAN RESUME LOGGING
            await checkout.buy(
                page=page,
            )
