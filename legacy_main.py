...
# This script is under constant development - currently in development trying to improve reliability for full local use 25.01.2026
    ...
    with Camoufox(disable_coop=True, window=(1280, 720), i_know_what_im_doing=True, geoip=True) as browser:
        ...
        while error_count <= 10:
            try:
                ...
                refresh_count = 0  # Prevent infinite loop if event never appears
                while True:
                    ...
                    print(f"Page refreshed at {CT} (attempt {refresh_count + 1})")
                    page.goto(f"{organiserURL}")
                    page.wait_for_load_state("networkidle", timeout=30000)  # Wait for page to fully load before checking elements, with 30s timeout
                ...
                if not ticketsReleased:
                    raise Exception("No available tickets found to reserve")
        ...
        while error_count <= 10:
            ...
            print("Payment process completed - check screenshot for result")
            break  # Exit checkout loop after attempting payment